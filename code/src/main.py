...
from model.transformer import available_models
...
df_train, df_val, df_test = df[df['train1_test0_val2'] == 1][use_columns], df[df['train1_test0_val2'] == 2][use_columns], df[
...
for idx, model_name_or_path in enumerate(available_models):
    ...
    print('loaded the %d-th model: \"%s\", may locate in \"%s\", and fine-tuned model will saved at \"%s\" if applicable' % (
    ...
    if to_do_what == 'evaluate':
        ...
        processor.rebuild_model(concise_vector_len=concise_vector_len).fin_tune(
        ...
        for remain_data in range(0, 10):
            ...
            sent_eb = processor.reload_model(best_model_used_to_infer_entire_pubmed).infer(df[['pm_id', 'content']].tolist(), infer_batch_size=320000, batch_size=640)
