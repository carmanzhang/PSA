...
desc table (
    ...
    select JSONExtractString(line, 'pm_id')                as pm_id,
           ...
           arrayMap(y1->
                        [JSONExtractString(y1, 'Id'), JSONExtractString(y1, 'Score')],
                    JSONExtractArrayRaw(
                            arrayFilter(x1->JSONExtractString(x1, 'LinkName') == 'pubmed_pubmed_reviews_five',
                                        JSONExtractArrayRaw(record, 'LinkSetDb'))[1], 'Link')
...
select pm_id,
       arrayStringConcat(
               arraySlice(arrayConcat(splitByChar(' ', clean_title), splitByChar(' ', clean_abstract)), 1, 200),
               ' ') as content
