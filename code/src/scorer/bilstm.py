...
class BiLSTMScorer(SupervisedScorer):
    ...
    def parsr_config_load_or_train(self):
        ...
        self.config = config
        _, _, test = ut.splits(self.config.nli_path)
        word_vectors = ut.load_vocab(self.config.vocab_path)
